...
apps/web/src
 ├ app/
 ├ components/
 ├ generator/      (Generative UI Engine helpers)
 ├ animations/
 ├ utils/
 └ types/
```
...
### Generative Components Folder

Instead of static predefined sections (e.g., `HeroSection.tsx`), Stolio uses dynamic meta-components.
The AI yields an AST composed of fundamental web building blocks:

`DynamicRenderer`: The orchestration wrapper.
`DynamicSection`: Handled layout (flex/grid) and spacing.
`DynamicText`: Renders typography with generated fonts, colors, and sizes.
`DynamicContainer`: Wrapper for shapes, glassmorphism, or cards.
`DynamicList`: For structured items like projects and skills.

These components receive structured JSON objects carrying Tailwind configuration strings and render visually distinct elements on the fly.
---
...
```
Generative UI JSON (AST)
 ↓
Dynamic Renderer Component
 ↓
Mapping AST nodes to Tailwind classes & Interactive Components
 ↓
Rendered Bespoke Portfolio Page
```

The rendering engine parses the recursive UI JSON object, applying dynamic tailwind classes and Framer Motion variants generated by the AI model.
